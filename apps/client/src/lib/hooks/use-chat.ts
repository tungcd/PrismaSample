...
export function useChatRoom(roomId: number) {
  console.log('[useChatRoom] Hook called with roomId:', roomId);
  ...
  const loadRoom = useCallback(async () => {
    console.log('[useChatRoom] loadRoom called for roomId:', roomId);
    try {
      setIsLoading(true);
      setError(null);
      ...
      console.log('[useChatRoom] Room data loaded:', {
        id: roomData.id,
        type: roomData.type,
      ...
      console.log('[useChatRoom] Messages loaded:', {
  ...
  useEffect(() => {
    ...
    console.log('[useChatRoom] Setting up message:new listener');

    const unsubscribe = onMessageNew((data) => {
      console.log('[useChatRoom] Received message:new', {
        roomId: data.roomId,
      ...
      if (data.roomId === roomId) {
        setMessages((prev) => {
          console.log('[useChatRoom] Processing message, current messages:', prev.length);
          
          // Avoid duplicates by checking if message with this ID already exists
          const exists = prev.find((m) => m.id === data.message.id);
          if (exists) {
            console.log('[useChatRoom] Message already exists, skipping');
            return prev;
          }
          
          // If this is our own message, find and replace the matching temporary message
          if (data.message.senderId === Number(user?.id)) {
            console.log('[useChatRoom] This is our message, looking for temp message to replace');
            ...
            if (tempIndex !== -1) {
              console.log('[useChatRoom] Found temp message at index', tempIndex, '- replacing');
              // Replace temporary message with real one
              const updated = [...prev];
              updated[tempIndex] = data.message;
              return updated;
            } else {
              console.log('[useChatRoom] No temp message found, adding new message');
            }
          }
          
          // Add new message (either from others or no matching temp found)
          console.log('[useChatRoom] Adding new message to list');
    ...
    return () => {
      console.log('[useChatRoom] Cleaning up message:new listener');
      unsubscribe();
    };
  ...
  const sendMessage = useCallback(
    async (content: string, replyToId?: number) => {
      ...
      console.log('[useChatRoom] sendMessage called', {
        roomId,
      ...
      try {
        ...
        console.log('[useChatRoom] Adding temp message', { tempMessageId: tempMessage.id });
        ...
        if (isConnected) {
          console.log('[useChatRoom] Sending via WebSocket');
          sendMessageSocket(roomId, content, replyToId);
        } else {
          // Fallback to REST API
          console.log('[useChatRoom] Fallback to REST API');
