...
export function ChatRoom({ roomId, onBack }: ChatRoomProps) {
  ...
  const handleSend = async (attachments?: Array<{ url: string; type: string; name: string }>) => {
    ...
    if (editingMessage) {
      ...
      // Note: The useChatRoom hook's sendMessage doesn't support attachments yet
      // We'll send via REST API if attachments exist
        ...
        try {
          await chatApi.sendMessage(roomId, {
            ...
            attachments: attachments.map((att) => ({
              ...
              fileName: att.name,
              fileType: att.type,
              fileSize: 0, // We don't track size in frontend for now
            })),
          });
        } catch (error) {
