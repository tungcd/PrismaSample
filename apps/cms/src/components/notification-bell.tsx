...
export function NotificationBell({ token }: NotificationBellProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell
          className={`h-5 w-5 ${isConnected ? "text-foreground" : "text-muted-foreground"}`}
        />
        {unreadCount > 0 && (
        ...
        {isConnected && (
          <span className="absolute -bottom-1 -right-1 bg-green-500 rounded-full h-2 w-2 border-2 border-background" />
        )}
      </button>
      ...
      {isOpen && (
        <>
          ...
          {/* Panel */}
          <div className="absolute right-0 mt-2 w-96 bg-card rounded-lg shadow-xl border z-50 max-h-[600px] flex flex-col">
            {/* Header */}
            ...
            <div className="overflow-y-auto flex-1">
              {isLoading ? (
                ...
                <div className="p-8 text-center text-muted-foreground">
                  <Bell className="h-12 w-12 mx-auto text-muted mb-2" />
